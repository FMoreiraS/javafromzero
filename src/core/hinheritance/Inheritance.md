...
print para também o salário do funcionário, usando a palavra super e *evitando a reescrita do
método*.
````
public class Employee extends Person{
    private double salary;
...
e às subclasses (em qualquer pacote). Se tentarmos usar um atributo como o *name* da classe Person em
um método da Employee, não conseguiremos, pois name é privado em Person. As únicas formas de acessá-lo sem usar
public é usando o método *get* (getName) ou mudando o modificador para *protected* (o que é mais breve).
````
public class Person {
